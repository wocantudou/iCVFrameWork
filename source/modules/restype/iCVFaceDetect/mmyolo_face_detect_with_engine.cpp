...
int32_t MmyoloFaceDetectWithEngine::preprocess_impl(const RESTYPE res_type,
    ...
    int model_input_h = inputs_.at(INPUT_NODE_NAME).dims_.d[2];
    int model_input_w = inputs_.at(INPUT_NODE_NAME).dims_.d[3];
    ...
    ret = DNNCVLIB::sub_mean_and_divide_std(
        ...
        inputs_.at(INPUT_NODE_NAME).data_.data(), mean_data.data(),
        std_data.data());
...
int MmyoloFaceDetectWithEngine::detect_face(const RESTYPE res_type,
    ...
    std::call_once(once_flag_, [this, res_type, &ret]() {
        ret = DnnWrapperInst()->prepare_inputs(res_type, inputs_);
    });
